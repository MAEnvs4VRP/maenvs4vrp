...
class PolicyNet(nn.Module):
    def __init__(
        self, nodes_stat_obs_dim, nodes_dyn_obs_dim, agents_obs_dim, embed_dim
    ):
        ...
        self.agents_embedding = nn.Linear(agents_obs_dim, embed_dim, bias=False)

        step_context_dim = 2 * embed_dim

        self.project_node_embeddings = nn.Linear(embed_dim, 3 * embed_dim, bias=False)

        self.project_agents_step_context = nn.Linear(
        ...
        self.veh_encoder_self_attention = MultiHeadAttentionProj(
            embed_dim=embed_dim, n_heads=8
        )
        self.veh_encoder_cross_attention = MultiHeadAttention(
            embed_dim=embed_dim, n_heads=8
        )
    ...
    def forward(
        ...
        agents_embed = self.agents_embedding(all_agents_obs)
        ...
        veh_embeddings = veh_embeddings + self.veh_encoder_self_attention(
            veh_embeddings, mask=active_agents_mask
        )
        ...
        veh_embeddings = self.veh_encoder_cross_attention(
            veh_embeddings, glimpse_K, glimpse_V, agents_action_mask.any(axis=1)
        )
