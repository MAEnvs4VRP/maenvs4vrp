...
class Environment(AECEnv):
    ...
    def check_solution_validity(self):
        ...
        )  # Time to serve node and get back to depot
        ...
        for ii in range(sorted_data.size(1)):
            ...
            curr_time = torch.max(
                curr_time + dist, gather_by_index(self.td_state["tw_low"], next_node)
            )
            assert torch.all(
                curr_time <= gather_by_index(self.td_state["tw_high"], next_node)
