...
class Environment(AECEnv):
    ...
    def reset(
        ...
        feasible_nodes = torch.ones(
            (*batch_size, self.num_nodes), dtype=torch.bool, device=self.device
        )
        feasible_nodes = torch.where(
            self.td_state["is_delivery"], False, feasible_nodes
        )
        ...
        self.td_state["agents"] = TensorDict(
            source={
                ...
                * torch.ones(
                ...
                "action_mask": feasible_nodes.unsqueeze(1).repeat(
                    (1, self.num_agents, 1)
                ),
                "pending_deliveries": torch.zeros(
                    (*batch_size, self.num_agents, self.num_nodes),
                    dtype=torch.bool,
                    device=self.device,
                ),
    ...
    def _update_curr_agent_feasibility(self):
        ...
        c3 = (
            self.td_state["cur_agent"]["cur_load"] + self.td_state["demands"]
    ...
    def _update_state(self, action):
        ...
        self.td_state["cur_agent", "action_mask"].scatter_(
            1, action_delivery_idx, is_pickup_bool
        )
        self.td_state["cur_agent", "action_mask"].scatter_(
        ...
        self.td_state["agents", "action_mask"].scatter_(
            1,
            self.td_state["cur_agent_idx"][:, :, None].expand(-1, -1, self.num_nodes),
            self.td_state["cur_agent", "action_mask"].unsqueeze(1),
        )

        # open corresponding delivery if action is pickup
        self.td_state["cur_agent", "pending_deliveries"].scatter_(
            1, action_delivery_idx, is_pickup_bool
        )
        self.td_state["cur_agent", "pending_deliveries"].scatter_(1, action, False)
        self.td_state["agents", "pending_deliveries"].scatter_(
            1,
            self.td_state["cur_agent_idx"][:, :, None].expand(-1, -1, self.num_nodes),
            self.td_state["cur_agent", "pending_deliveries"].unsqueeze(1),
        )
        ...
        pending_mask = (
            ...
            * ~self.td_state["cur_agent", "pending_deliveries"]
        )
        self.td_state["nodes", "active_nodes_mask"] = torch.where(
            agents_done, pending_mask, self.td_state["nodes", "active_nodes_mask"]
        )
        ...
        self.td_state["cur_agent"] = TensorDict(
            {
                ...
                .squeeze(1)
                .clone(),
                "pending_deliveries": self.td_state["agents"]["pending_deliveries"]
                .gather(
                    1,
                    self.td_state["cur_agent_idx"][:, :, None].expand(
                        -1, -1, self.num_nodes
