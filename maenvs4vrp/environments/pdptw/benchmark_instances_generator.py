...
class BenchmarkInstanceGenerator(InstanceBuilder):
    ...
    def get_list_of_benchmark_instances(cls):
        ...
        return {
            "Li&Lim": [
                s.split(".")[0]
                for s in os.listdir(
                    path.join(base_dir, BENCHMARK_INSTANCES_PATH, "Li&Lim")
                )
            ]
        }
    ...
    def __init__(
        ...
        assert instance_type in ["Li&Lim"], "instance unknown type"
        assert (
            len(set_of_instances) > 0
    ...
    def read_instance_data(self, instance_name: str) -> Dict:
        ...
        benchmark_file = "{path_to_benchmark_instances}/{instance}.txt".format(
            path_to_benchmark_instances=path_to_file, instance=instance_name
        )
    ...
    def parse_instance_data(self, instance_data: list, instance_name: str) -> Dict:
        ...
        depot_idx = 0
        data["depot_idx"] = depot_idx * torch.ones(
            (*self.batch_size, 1), dtype=torch.int64, device=self.device
        )
        data["coords"] = torch.tensor(
            coords, dtype=torch.float, device=self.device
        ).unsqueeze(0)
        data["demands"] = torch.tensor(
            demands, dtype=torch.float, device=self.device
        ).unsqueeze(0)
        time_windows = torch.tensor(
            time_windows, dtype=torch.float, device=self.device
        ).unsqueeze(0)
        data["tw_low"] = time_windows[:, :, 0].clone()
        data["tw_high"] = time_windows[:, :, 1].clone()

        data["service_time"] = torch.tensor(
            service_time, dtype=torch.float, device=self.device
        ).unsqueeze(0)
        data["start_time"] = (
            time_windows[:, :, 0]
            .gather(
        ...
        data["speed"] = torch.ones(
            (*self.batch_size, 1), dtype=torch.float, device=self.device
        )

        data["is_depot"] = torch.zeros(
            (*self.batch_size, instance["num_nodes"]),
            dtype=torch.bool,
            device=self.device,
        )
        data["is_depot"][:, depot_idx] = True
        data["capacity"] = capacity * torch.ones(
            (*self.batch_size, 1), dtype=torch.float, device=self.device
        )

        data["pickup_idx"] = torch.tensor(
            pickup_idx, dtype=torch.long, device=self.device
        ).unsqueeze(0)
        data["delivery_idx"] = torch.tensor(
            delivery_idx, dtype=torch.long, device=self.device
        ).unsqueeze(0)
    ...
    def random_sample_instance(
        ...
        pickup_idx = data["pickup_idx"][
            torch.nonzero(data["pickup_idx"], as_tuple=True)
        ]
        ...
        delivery_idx = torch.arange(
            num_services + 1, new_instance["num_nodes"], device=self.device
        ).repeat(self.batch_size.numel(), 1)

        new_data["pickup_idx"] = torch.zeros(
            (*self.batch_size, new_instance["num_nodes"]),
            dtype=torch.long,
            device=self.device,
        )
        new_data["delivery_idx"] = torch.zeros(
            (*self.batch_size, new_instance["num_nodes"]),
            dtype=torch.long,
            device=self.device,
        )

        new_data["pickup_idx"].scatter_(
        ...
        new_data["delivery_idx"].scatter_(
...
if __name__ == "__main__":
    generator = BenchmarkInstanceGenerator(
        instance_type="Li&Lim", set_of_instances={"lc101"}
    )
