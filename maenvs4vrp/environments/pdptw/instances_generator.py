...
class InstanceGenerator(InstanceBuilder):
    ...
    def get_time_windows(
        ...
        time_windows = torch.zeros(
            (*batch_size, self.max_num_nodes, 2), device=self.device
        )

        depot_coord = instance["coords"].gather(
            1, instance["depot_idx"][:, :, None].expand(-1, -1, 2)
        )
        pickup_coords = instance["coords"].gather(
            1, pickup_idx[:, :, None].expand(-1, -1, 2)
        )
        delivery_coords = instance["coords"].gather(
            1, delivery_idx[:, :, None].expand(-1, -1, 2)
        )
        ...
        time_centers = inf.squeeze(-1) + torch.rand(
            *batch_size, num_services, device=self.device
        ) * (sup - inf).squeeze(-1)
        time_half_width = torch.empty(
            (*batch_size, num_services), device=self.device
        ).uniform_(self.service_times / 2, depot_end / 3)

        time_windows[:, :, 0] = time_windows[:, :, 0].scatter(
            1,
            pickup_idx,
            torch.clip(time_centers - time_half_width, depot_start, depot_end),
        )
        time_windows[:, :, 1] = time_windows[:, :, 1].scatter(
            1,
            pickup_idx,
            torch.clip(time_centers + time_half_width, depot_start, depot_end),
        )

        dist_pickup_delivery = torch.pairwise_distance(
            delivery_coords, pickup_coords, keepdim=False
        )
        ...
        time_windows[:, :, 0] = time_windows[:, :, 0].scatter(
            ...
            delivery_idx,
            torch.clip(
                time_centers + dist_pickup_delivery - time_half_width,
                depot_start,
                depot_end,
            ),
        )
    ...
    def random_generate_instance(
        ...
        if num_nodes is not None:
            assert num_nodes > 0, "number of nodes must be grater them 0!"
            assert (num_nodes - 1) % 2 == 0, "number of services must be even"
        ...
        num_services = (self.max_num_nodes - 1) // 2
        pickup_idx = torch.arange(1, num_services + 1, device=self.device).repeat(
            self.batch_size.numel(), 1
        )
        delivery_idx = torch.arange(
            num_services + 1, self.max_num_nodes, device=self.device
        ).repeat(self.batch_size.numel(), 1)

        instance["pickup_idx"] = torch.zeros(
            (*self.batch_size, self.max_num_nodes), dtype=torch.long, device=self.device
        )
        instance["delivery_idx"] = torch.zeros(
            (*self.batch_size, self.max_num_nodes), dtype=torch.long, device=self.device
        )

        instance["pickup_idx"].scatter_(
            1, delivery_idx, pickup_idx
        )  # For pickup tasks, the PICKUP index is 0, whereas the DELIVERY sibling gives the index of the corresponding delivery task
        instance["delivery_idx"].scatter_(
            1, pickup_idx, delivery_idx
        )  # For delivery tasks, the PICKUP index gives the index of the corresponding pickup task.

        instance["is_pickup"] = torch.zeros(
            (*self.batch_size, self.max_num_nodes), dtype=torch.bool, device=self.device
        )
        instance["is_pickup"].scatter_(1, pickup_idx, True)

        instance["is_delivery"] = torch.zeros(
            (*self.batch_size, self.max_num_nodes), dtype=torch.bool, device=self.device
        )
        ...
        demands = torch.randint(
            low=1,
            high=11,
            size=(*self.batch_size, num_services),
            dtype=torch.float,
            device=self.device,
        )
        instance["demands"] = torch.zeros(
            (*self.batch_size, self.max_num_nodes),
            dtype=torch.float,
            device=self.device,
        )
        ...
        time_windows = self.get_time_windows(
            instance, pickup_idx, delivery_idx, self.batch_size, seed
        )
