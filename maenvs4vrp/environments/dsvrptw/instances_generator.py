...
class InstanceGenerator(InstanceBuilder):
    ...
    def random_generate_instance(
        ...
        locs = torch.randint(
            *cust_loc_range, (*self.batch_size, num_nodes, 2), dtype=torch.float
        )
        # Sample demands             q_j ~ U(5,  40)
        demands = torch.randint(
            *cust_dem_range, (*self.batch_size, num_nodes), dtype=torch.float
        )
        demands[:, self.depot_idx] = 0.0

        # Sample service_times       s_j ~ U(10, 30)
        service_times = torch.randint(
            *service_times_range, (*self.batch_size, num_nodes), dtype=torch.float
        )
        ...
        if isinstance(dod, float):
            ...
            ratio = torch.tensor(dod)[
                torch.randint(0, len(dod), (batch_size,), dtype=torch.int64)
            ]
        ...
        aprs = is_dyn * is_dyn_e * torch.randint(
            1, horizon // 3 + 1, size, dtype=torch.float
        ) + is_dyn * (1 - is_dyn_e) * torch.randint(
            horizon // 3 + 1, 2 * horizon // 3 + 1, size, dtype=torch.float
        )
        ...
        if isinstance(tw_ratio, float):
            ...
            ratio = torch.tensor(tw_ratio)[
                torch.randint(0, len(tw_ratio), (batch_size,), dtype=torch.int64)
            ]
            has_tw = ratio[:, None, None].expand(*size).bernoulli()

        # Sample TW width        tw_j = H if not in TW subset
        #                        tw_j ~ U(30,90) if in TW subset
        tws = (1 - has_tw) * torch.full(size, horizon) + has_tw * torch.randint(
            *cust_tw_range, size, dtype=torch.float
        )
        ...
        tt_0j = (
            (depot_locs.unsqueeze(1) - cust_locs).pow(2).sum(-1).pow(0.5)
        ...
        rdys = has_tw * (
            aprs
            + torch.rand(size)
            * (horizon - torch.max(tt_0j + service_times, tws) - aprs)
        )
        rdys.floor_()
        ...
        instance["end_time"] = horizon * torch.ones(
            *self.batch_size, dtype=torch.int64, device=self.device
        )
