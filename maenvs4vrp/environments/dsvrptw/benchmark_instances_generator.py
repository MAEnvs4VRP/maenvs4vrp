...
BENCHMARK_INSTANCES_PATH = "dsvrptw/data/benchmark"


class BenchmarkInstanceGenerator(InstanceBuilder):
    """
    DSVRPTW benchmark instance generation class.
    """
    ...
    def get_list_of_benchmark_instances(cls):
        ...
        return {
            "Solomon": [
                s.split(".")[0]
                for s in os.listdir(
                    path.join(base_dir, BENCHMARK_INSTANCES_PATH, "Solomon")
                )
            ],
            "Homberger": [
                s.split(".")[0]
                for s in os.listdir(
                    path.join(base_dir, BENCHMARK_INSTANCES_PATH, "Homberger")
                )
            ],
        }
    ...
    def __init__(
        ...
        assert instance_type in ["Solomon", "Homberger"], "instance unknown type"
        assert (
    ...
    def random_sample_instance(
        ...
        index = torch.cat(
            [
                idxs[data["is_depot"]],
                idxs[~data["is_depot"]][torch.randperm(instance["num_nodes"] - 1)],
            ],
            dim=0,
        )[: new_instance["num_nodes"]]
    ...
    def sample_instance(
        ...
        if sample_type == "random":
            ...
            # sample first n
            instance = self.sample_first_n_services(
                instance_name=instance_name,
                num_agents=num_agents,
                num_nodes=num_nodes,
                device=device,
...
if __name__ == "__main__":
    generator = BenchmarkInstanceGenerator(
        instance_type="Solomon", set_of_instances={"C101"}
    )
