...
class Environment(AECEnv):
    ...
    def _sample_speed(self):
        ...
        """
        late = (
            torch.empty((*self.batch_size, 1)).bernoulli_(self.late_p).to(self.device)
        )
        rand = torch.randn_like(late).to(self.device)
        speed = late * self.slow_down * (1 + self.late_var * rand) + (1 - late) * (
            1 + self.speed_var * rand
        )
    ...
    def reset(
        ...
        cust_mask = self.td_state["appear_time"] <= self.td_state[
            "start_time"
        ].unsqueeze(-1)
        ...
        self.td_state["agents"] = TensorDict(
            source={
                ...
                * torch.ones(
                ...
                "cur_tdist": torch.zeros(
                    (*batch_size, self.num_agents),
                    dtype=torch.float,
                    device=self.device,
                ),
                "cum_tdist": torch.zeros(
                    (*batch_size, self.num_agents),
                    dtype=torch.float,
                    device=self.device,
                ),
                "cur_penalty": torch.zeros(
                    (*batch_size, self.num_agents),
                    dtype=torch.float,
                    device=self.device,
                ),
                "cum_penalty": torch.zeros(
                    (*batch_size, self.num_agents),
                    dtype=torch.float,
                    device=self.device,
                ),
    ...
    def _update_curr_agent_feasibility(self):
        ...
        cust_mask = (
            self.td_state["appear_time"] <= self.td_state["cur_agent"]["cur_time"]
        )
    ...
    def _update_state(self, action):
        ...
        self.td_state["agents"]["cur_penalty"].scatter_(
            1, self.td_state["cur_agent_idx"], self.td_state["cur_agent"]["cur_penalty"]
        )
        self.td_state["agents"]["cum_penalty"].scatter_(
            1, self.td_state["cur_agent_idx"], self.td_state["cur_agent"]["cum_penalty"]
        )
        ...
        self.td_state["agents"]["cur_tdist"].scatter_(
            1, self.td_state["cur_agent_idx"], self.td_state["cur_agent"]["cur_tdist"]
        )
        self.td_state["agents"]["cum_tdist"].scatter_(
            1, self.td_state["cur_agent_idx"], self.td_state["cur_agent"]["cum_tdist"]
        )
        ...
        self.td_state["cur_agent"] = TensorDict(
            {
                ...
                .gather(1, self.td_state["cur_agent_idx"])
                .clone(),
                "cur_tdist": self.td_state["agents"]["cur_tdist"]
                .gather(1, self.td_state["cur_agent_idx"])
                .clone(),
                "cum_tdist": self.td_state["agents"]["cum_tdist"]
                .gather(1, self.td_state["cur_agent_idx"])
                .clone(),
                "cur_penalty": self.td_state["agents"]["cur_penalty"]
                .gather(1, self.td_state["cur_agent_idx"])
                .clone(),
                "cum_penalty": self.td_state["agents"]["cum_penalty"]
                .gather(1, self.td_state["cur_agent_idx"])
                .clone(),
    ...
    def check_solution_validity(self):
        ...
        for ii in range(sorted_data.size(1)):
            ...
            assert torch.all(
                used_cap <= self.td_state["capacity"]
