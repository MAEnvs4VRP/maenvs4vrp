...
class BenchmarkInstanceGenerator(InstanceBuilder):
    ...
    def get_list_of_benchmark_instances(cls):
        ...
        return {
            "Vidal": [
                s.split(".")[0]
                for s in os.listdir(
                    path.join(base_dir, BENCHMARK_INSTANCES_PATH, "Vidal")
                )
            ]
        }
    ...
    def __init__(
        ...
        assert instance_type in ["Vidal"], "instance unknown type"
        assert (
    ...
    def parse_instance_data(self, instance_data: list, instance_name: str) -> Dict:
        ...
        for data in instance_data[
            (instance["num_depots"] + 1) : instance["num_nodes"] + 1
        ]:
            coords.append([float(data[1]), float(data[2])])
            demands.append(float(data[4]))
            time_windows.append([float(data[-2]), float(data[-1])])
        ...
        self.depot_idx = torch.arange(
            0, instance["num_depots"], device=self.device
    ...
    def random_sample_instance(
        ...
        index = torch.cat(
            [
                idxs[data["is_depot"]],
                idxs[~data["is_depot"]][
                    torch.randperm(instance["num_nodes"] - instance["num_depots"])
                ],
            ],
            dim=0,
...
if __name__ == "__main__":
    generator = BenchmarkInstanceGenerator(
        instance_type="Vidal", set_of_instances={"pr11a"}
    )
