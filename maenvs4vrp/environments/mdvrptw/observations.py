...
class Observations(ObservationBuilder):
    ...
    def get_feat_agent_arrivedepot_div_end_time(self):
        ...
        ptime = self.env.td_state["cur_agent"]["cur_time"].clone()
        depot_loc = self.env.td_state["coords"].gather(
            1, self.env.td_state["cur_agent"]["depot_idx"][:, :, None].expand(-1, -1, 2)
        )
    ...
    def get_feat_other_agents_dist2depot_div_end_time(self):
        ...
        feat = torch.cdist(
            self.env.td_state["depot_loc"],
            locs,
            p=2,
            compute_mode="donot_use_mm_for_euclid_dist",
        )
        feat = feat.gather(
            1, self.env.td_state["agents", "depot_idx"].unsqueeze(1)
        ).squeeze(1)
        return feat / self.env.td_state["end_time"].unsqueeze(dim=-1)
