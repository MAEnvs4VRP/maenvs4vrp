...
class Environment(AECEnv):
    ...
    def _update_curr_agent_feasibility(self):
        ...
        c2 = service_startj + self.td_state[
            "service_time"
        ] + time2depot <= self.td_state["end_time"].unsqueeze(-1)
    ...
    def check_solution_validity(self):
        ...
        curr_node = (
            self.td_state["depot_idx"][:, 0]
            if num_depots > 1
            else torch.zeros(*self.batch_size, dtype=torch.int64, device=self.device)
        )
        ...
        agent_indices = torch.gather(
            self.td_state["solution"]["agents"], dim=-1, index=sorted_indices
        )
