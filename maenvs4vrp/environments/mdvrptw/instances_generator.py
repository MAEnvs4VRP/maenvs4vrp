...
class InstanceGenerator(InstanceBuilder):
    ...
    def get_time_windows(
        ...
        depot_coord = (
            instance["coords"]
            .gather(1, instance["depot_idx"][:, :, None].expand(-1, -1, 2))
            .mean(dim=1)
            .unsqueeze(1)
        )
        dist_depot = torch.pairwise_distance(
            depot_coord, instance["coords"], keepdim=True
        )
        ...
        time_centers = inf.squeeze(-1) + torch.rand(
            *batch_size, self.max_num_nodes, device=self.device
        ) * (sup - inf).squeeze(-1)
        time_half_width = torch.empty(
            (*batch_size, self.max_num_nodes), device=self.device
        ).uniform_(self.service_times / 2, depot_end / 3)
        time_windows[:, :, 0] = torch.clip(
            time_centers - time_half_width, depot_start, depot_end
        )
        time_windows[:, :, 1] = torch.clip(
            time_centers + time_half_width, depot_start, depot_end
        )
    ...
    def random_generate_instance(
        ...
        if num_depots is not None:
            assert num_depots > 0, "number depots must be grater them 0!"
    ...
    for num_nodes, n_agent, num_depots in [(105, 5, 5), (55, 5, 5)]:
        ...
        for k in range(number_instances):
            instance = generator.sample_instance(
                num_agents=n_agent, num_nodes=num_nodes, num_depots=num_depots
            )
            name = f"generated_val_servs_{num_nodes - 1}_agents_{n_agent}_depots_{num_depots}_{k}"
            instance["name"] = name
            if not os.path.exists(
                f"data/generated/servs_{num_nodes - num_depots}_agents_{n_agent}_depots_{num_depots}/validation"
            ):
                os.makedirs(
                    f"data/generated/servs_{num_nodes - num_depots}_agents_{n_agent}_depots_{num_depots}/validation"
                )
            ...
            instance["name"] = name
            if not os.path.exists(
                f"data/generated/servs_{num_nodes - num_depots}_agents_{n_agent}_depots_{num_depots}/test"
            ):
                os.makedirs(
                    f"data/generated/servs_{num_nodes - num_depots}_agents_{n_agent}_depots_{num_depots}/test"
                )
