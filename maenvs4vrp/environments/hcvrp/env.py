...
class Environment(AECEnv):
    ...
    def reset(
        ...
        self.td_state["agents"] = TensorDict(
            source={
                ...
                * torch.ones(
                ...
                "cur_length": torch.zeros(
                    (*batch_size, self.num_agents),
                    dtype=torch.float,
                    device=self.device,
                ),
                "cum_length": torch.zeros(
                    (*batch_size, self.num_agents),
                    dtype=torch.float,
                    device=self.device,
                ),
                "trip_count": torch.zeros(
                    (*batch_size, self.num_agents),
                    dtype=torch.int64,
                    device=self.device,
                ),
        ...
        self.td_state["solution"] = TensorDict(
    ...
    def _update_done(self, action):
        ...
        was_last = (
            self.td_state["agents"]["last_was_depot"]
        ...
        self.td_state["agents"]["active_agents_mask"].scatter_(
            1, self.td_state["cur_agent_idx"], new_active
        )
        # update last_was_depot flag for agent
        self.td_state["agents"]["last_was_depot"].scatter_(
            1, self.td_state["cur_agent_idx"], is_depot
        )
    ...
    def _update_state(self, action):
        ...
        self.td_state["agents"]["cur_length"].scatter_(
            1, self.td_state["cur_agent_idx"], self.td_state["cur_agent"]["cur_length"]
        )
        self.td_state["agents"]["cum_length"].scatter_(
            1, self.td_state["cur_agent_idx"], self.td_state["cur_agent"]["cum_length"]
        )
        ...
        if is_depot.any():
            ...
            self.td_state["agents"]["trip_count"].scatter_add_(
                1, self.td_state["cur_agent_idx"], is_depot.float().to(torch.int64)
            )
        ...
        self.td_state["cur_agent"]["cur_load"] -= self.td_state["demands"].gather(
            1, action
        )
        ...
        self.td_state["cur_agent"] = TensorDict(
            {
                ...
                .squeeze(1),
                "capacity": self.td_state["agents"]["capacity"]
                .gather(1, self.td_state["cur_agent_idx"])
                .clone(),
                "speed": self.td_state["agents"]["speed"]
                .gather(1, self.td_state["cur_agent_idx"])
                .clone(),
                ...
                .gather(1, self.td_state["cur_agent_idx"])
                .clone(),
                "cur_length": self.td_state["agents"]["cur_length"]
                .gather(1, self.td_state["cur_agent_idx"])
                .clone(),
                "cum_length": self.td_state["agents"]["cum_length"]
                .gather(1, self.td_state["cur_agent_idx"])
                .clone(),
    ...
    def check_solution_validity(self):
        ...
        visited_nodes = torch.zeros(
            batch_size, self.num_nodes, dtype=torch.int64, device=self.device
        )
        ...
        for ii in range(sorted_actions.size(1)):
            ...
            current_load = torch.where(
                is_depot, torch.zeros_like(current_load), current_load
            )
        ...
        assert torch.all(
            visited_nodes_exc_depot == 1
        ), "Some nodes were missed or visited multiple times!"
