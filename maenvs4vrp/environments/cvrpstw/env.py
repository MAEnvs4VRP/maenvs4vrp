...
class Environment(AECEnv):
    ...
    def reset(
        ...
        self.td_state["Pmax"] = instance_info["Pmax"] * self.td_state[
            "max_tour_duration"
        ].unsqueeze(
            -1
        )  # transform to time
        self.td_state["Wmax"] = instance_info["Wmax"] * self.td_state[
            "max_tour_duration"
        ].unsqueeze(
            -1
        )  # transform to time
        ...
        self.td_state["tw_high_limit"] = (
            self.td_state["tw_high"] + self.td_state["Pmax"]
        )
        self.td_state["arrive_limit"] = (
            self.td_state["tw_low"] - self.td_state["Pmax"] - self.td_state["Wmax"]
        )
    ...
    def _update_curr_agent_feasibility(self):
        ...
        c0 = (
            arrivej > self.td_state["arrive_limit"]
        )  # agents can only arrive at each customer after $o_i - P_{max} - W_{max}$
    ...
    def _update_state(self, action):
        ...
        penalty = -(
            self.early_penalty * torch.clip(tw_low - arrivej, min=0, max=None)
    ...
    def check_solution_validity(self):
        ...
        for ii in range(sorted_data.size(1)):
            ...
            assert torch.all(
                arrivej > arrive_limit
            ), "Arrived before allowed limit at customer node."
            # c1: service must start before tw_high_limit
            assert torch.all(
                service_startj <= tw_high_limit
