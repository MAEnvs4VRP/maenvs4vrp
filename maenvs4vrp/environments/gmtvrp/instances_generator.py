...
class InstanceGenerator(InstanceBuilder):
            ...
            if self.variant_preset in ("all", "cvrp", "single_feat", "single_feat_otw"):
                ...
                if self.variant_preset == "single_feat_otw":
                    keep_mask = torch.zeros(
                        (*self.batch_size, 6), dtype=torch.bool
                    )  # O, TW, L, B, OTW, nothing
                    ...
                    keep_mask = torch.zeros(
                        (*self.batch_size, 5), dtype=torch.bool
                    )  # O, TW, L, B, nothing
                    keep_mask[torch.arange(*self.batch_size), indexes] = True
    ...
    def random_generate_instance(
        ...
        instance["initial_load"] = torch.full(
            (*self.batch_size, self.num_agents), initial_load, dtype=torch.float32
        )
            ...
            for k in range(number_instances):
                ...
                if problem not in MIXED_PROBLEMS:
                    instance = generator.sample_instance(
                        num_agents=n_agent, num_nodes=num_nodes, variant_preset=problem
                    )
                else:
                    if problem == "ovrpmb":
                        instance = generator.sample_instance(
                ...
                if not os.path.exists(
                    ...
                    print(
                        f"Creating directory: data/generated/servs_{num_nodes - 1}_agents_{n_agent}/{problem}/validation"
                    )
                with open(
                    f"data/generated/servs_{num_nodes - 1}_agents_{n_agent}/{problem}/validation/"
                    + name
                    ...
                    print(
                        f"Dumped data into: data/generated/servs_{num_nodes - 1}_agents_{n_agent}/{problem}/validation/{name}.pkl"
                    )
                ...
                if not os.path.exists(
                    ...
                    print(
                        f"Creating directory: data/generated/servs_{num_nodes - 1}_agents_{n_agent}/{problem}/test"
                    )
                with open(
                    f"data/generated/servs_{num_nodes - 1}_agents_{n_agent}/{problem}/test/"
                    + name
                    ...
                    pickle.dump(instance, fp, protocol=pickle.HIGHEST_PROTOCOL)
                    print(
                        f"Dumped data into: data/generated/servs_{num_nodes - 1}_agents_{n_agent}/{problem}/test/{name}.pkl"
                    )
