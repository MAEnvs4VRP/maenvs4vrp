...
class DenseReward(RewardFn):
    ...
    def get_reward(self, action):
        ...
        dist_depot2nodes = torch.pairwise_distance(
            self.env.td_state["depot_loc"],
            self.env.td_state["coords"],
            eps=0,
            keepdim=False,
        )
        ...
        if self.env.n_digits is not None:
            dist_depot2nodes = (
                torch.floor(self.env.n_digits * dist_depot2nodes) / self.env.n_digits
            )
            time_depot2nodes = (
                torch.floor(self.env.n_digits * time_depot2nodes) / self.env.n_digits
            )

        penalty[is_last_step] = self.pending_penalty * (
...
class DenseRewardV(RewardFn):
    ...
    def get_reward(self, action):
        ...
        penalty[is_last_step] = (
            ...
            * (
                (self.env.td_state["nodes"]["active_nodes_mask"])
                .sum(-1, keepdim=True)
                .float()[is_last_step]
            )
            - self.env.td_state["agents"]["cur_step"]
            .gt(0)
            .sum(-1)
            .float()
            .unsqueeze(-1)[is_last_step]
        )
