    ...
    * v0: Initial versions release (0.1.0)
    ...
    def reset(
        ...
        self.td_state["agents"] = TensorDict(
            source={
                "cum_profit": torch.zeros(
                    (*batch_size, self.num_agents),
                    dtype=torch.float,
                    device=self.device,
                ),
                "step_profit": torch.zeros(
                    (*batch_size, self.num_agents),
                    dtype=torch.float,
                    device=self.device,
                ),
    ...
    def reset_observe(
        ...
        seed: int | None = None,
        device: Optional[str] = "cpu",
        obs_list: Optional[List[str]] = ["nodes_static", "all_agents_action_mask"],
    ) -> TensorDict:
        """
        Resets the environment and sets the current agent.
    ...
    def reset_agent_select_observe(
        ...
        seed: int | None = None,
        device: Optional[str] = "cpu",
        obs_list: Optional[List[str]] = ["nodes_static", "action_mask", "agent"],
    ) -> TensorDict:
        """
        Resets the environment and sets the current agent.
    ...
    def _update_curr_agent_feasibility(self):
        ...
        c1 = service_startj + self.td_state["service_time"] + self.td_state[
            "time2depot"
    ...
    def _update_all_agents_feasibility(self):
        ...
        active_nodes = self.td_state["nodes"]["active_nodes_mask"].unsqueeze(
            1
        )  # [B, 1, N]
        ...
        _mask = (
            active_nodes & curr_agent_masks
        )  # Use & for boolean, * also works but & is clearer
        ...
        time2depot = distance_to_depot / self.td_state["speed"].unsqueeze(
            -1
        )  # [B, 1, N]
        ...
        c_time = service_startj + service_time + time2depot <= end_time.unsqueeze(
            -1
        ).unsqueeze(
            -1
        )  # [B, num_agents, N]
    ...
    def _update_state(self, action):
        ...
        # update agent profit
        self.td_state["cur_agent"]["cum_profit"] += self.td_state["profits"].gather(
            1, action
        )
        self.td_state["agents"]["cum_profit"].scatter_(
            1, self.td_state["cur_agent_idx"], self.td_state["cur_agent"]["cum_profit"]
        )
        self.td_state["cur_agent"]["step_profit"] = self.td_state["profits"].gather(
            1, action
        )
        self.td_state["agents"]["step_profit"].scatter_(
            1, self.td_state["cur_agent_idx"], self.td_state["cur_agent"]["step_profit"]
        )
        ...
        self.td_state["agents"]["visited_nodes"][
            r, self.td_state["cur_agent_idx"], action.squeeze(-1)
        ] = True
        # update agent step
        # update agent step
        ...
        self.td_state["cur_agent"] = TensorDict(
            {
                ...
                .squeeze(1)
                .clone(),
                "cum_profit": self.td_state["agents"]["cum_profit"]
                .gather(1, self.td_state["cur_agent_idx"])
                .clone(),
                "step_profit": self.td_state["agents"]["step_profit"]
                .gather(1, self.td_state["cur_agent_idx"])
                .clone(),
    ...
    def check_solution_validity(self):
        ...
        for ii in range(sorted_data.size(1)):
            ...
            assert torch.all(
                arrivej + service_time + time2depot
                <= self.td_state["end_time"].unsqueeze(-1)
