...
class Observations(ObservationBuilder):
    ...
    def get_feat_agent_frac_current_profit(self):
        ...
        """
        feat = self.env.td_state["cur_agent"]["cum_profit"] / self.env.td_state[
            "profits"
    ...
    def get_feat_other_agents_frac_time_left(self):
        ...
        Returns:
            npt.NDArray: agents fraction of elapsed time.
        """
        feats = 1 - self.env.td_state["agents"]["cur_time"] / self.env.td_state[
            "end_time"
        ].unsqueeze(dim=-1)
        return feats
    ...
    def get_feat_other_agents_frac_current_profit(self):
        ...
        """
        feats = self.env.td_state["agents"]["cum_profit"] / self.env.td_state[
            "profits"
        ].sum(dim=-1).unsqueeze(1)
        return feats
