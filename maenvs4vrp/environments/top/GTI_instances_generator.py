...
class GTIGenerator(InstanceBuilder):
    ...
    def get_list_of_benchmark_instances(cls):
        ...
        return {
            "GTI_20": [
                s.split(".")[0]
                for s in os.listdir(
                    path.join(base_dir, GENERATED_INSTANCES_PATH, "GTI")
                )
                if "20_L2_" in s
            ],
            "GTI_50": [
    ...
    def __init__(
        ...
        assert instance_type in ["GTI"], "instance unknown type"
    ...
    def read_instance_data(self, instance_name: str) -> Dict:
        ...
        generated_file = "{path_to_generated_instances}/{instance}.pkl".format(
            path_to_generated_instances=path_to_file, instance=instance_name
        )
    ...
    def parse_instance_data(self, instance_data: list, instance_name: str) -> Dict:
        ...
        for data in instance_data:
            coords.append(
                torch.cat((data["depot"][None, :], data["loc"]), -2).unsqueeze(0)
            )
            profits.append(
                torch.cat((torch.tensor([0]), data["prize"]), -1).unsqueeze(0)
            )
        ...
        self.depot_idx = 0
        data["depot_idx"] = self.depot_idx * torch.ones(
