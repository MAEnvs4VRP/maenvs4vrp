...
class Environment(AECEnv):
    ...
    def _update_all_agents_feasibility(self):
        ...
        waitj = torch.clip(
            self.td_state["tw_low"].unsqueeze(1) - arrivej, min=0
        )  # [B, num_agents, N]
        service_startj = arrivej + waitj  # [B, num_agents, N]

        c1 = service_startj <= self.td_state["tw_high"].unsqueeze(
            1
        )  # [B, num_agents, N]
        c2 = service_startj + self.td_state["service_time"].unsqueeze(
            1
        ) + self.td_state["time2depot"].unsqueeze(1) <= self.td_state[
            "end_time"
        ].unsqueeze(
            -1
        ).unsqueeze(
        ...
        # Capacity constraints
        c3 = self.td_state["demands"].unsqueeze(1) <= self.td_state["agents"][
            "cur_load"
        ].unsqueeze(
            -1
        )  # [B, num_agents, N]
        ...
        done_rows = done.squeeze(-1).nonzero(as_tuple=True)[
            0
        ]  # indices of rows where all agents inactive
        ...
        if self.force_visit:
            ...
            depot_open = _mask.gather(2, depot_idx_expanded) & ~must_leave.unsqueeze(
                -1
            )  # [B, num_agents, 1]
