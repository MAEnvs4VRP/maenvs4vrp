...
class InstanceGenerator(InstanceBuilder):
            ...
            if mixed:
                for problem_type in VARIANT_PRESETS:
                    val_path = os.path.join(
                        GENERATED_INSTANCES_PATH, folder, problem_type, "validation"
                    )
                    test_path = os.path.join(
                        GENERATED_INSTANCES_PATH, folder, problem_type, "test"
                    )
    ...
    def __init__(
        ...
        assert (
            ...
            or instance_type == ""
        ), "Instance type must be 'test', 'validation', '' or None."  # If None or empty, it loads both test and validation
    ...
    def generate_distance_limit(
        ...
        """
        max_dist = torch.max(
            torch.cdist(coords[:, 0:1], coords[:, 1:]).squeeze(-2), dim=1
        )[0]
        ...
        return torch.distributions.Uniform(
            dist_lower_bound, max_distance_limit
        ).sample()[..., None]
    ...
    def generate_time_windows(
        ...
        batch_size, n_loc = (
            coords.shape[0],
            coords.shape[1] - self.num_depots,
        )  # no depot
    ...
    def random_generate_instance(
        ...
        if num_depots is not None:
            assert num_depots > 0, "Number of depots must be greater than 0!"
        if num_agents is not None:
        ...
        if max_time is not None:
            assert max_time > 0, "Service times must be greater than 0!"
        if max_distance_limit is not None:
            assert max_distance_limit > 0, "Distance limit must be greater than 0!"
        ...
        self.depot_idx = torch.arange(self.num_depots, device=self.device).repeat(
            (*self.batch_size, 1)
        )
        instance["depot_idx"] = self.depot_idx

        # Coords unfiform generation
        coords = torch.FloatTensor(*self.batch_size, num_nodes, 2).uniform_(
        ...
        vehicle_capacity = torch.full(
            (*self.batch_size, 1), self.capacity, dtype=torch.float32
        )
        ...
        # Demands
        linehaul_demands, backhaul_demands = self.generate_demands(
            batch_size=self.batch_size, num_nodes=self.num_nodes
        )
        ...
        backhaul_class = self.generate_backhaul_class(
            shape=(*self.batch_size, 1), sample=self.sample_backhaul_class
        )
        ...
        # Distance limits
        distance_limits = self.generate_distance_limit(
            shape=(*self.batch_size, 1), coords=coords
        )
        instance["distance_limits"] = distance_limits

        # Initial load
        instance["initial_load"] = torch.full(
            (*self.batch_size, self.num_agents_total), initial_load, dtype=torch.float32
        )

        if self.subsample:
            instance = self.subsample_variant(
                td=instance, variant_preset=self.variant_preset
            )
    ...
    def augment_generate_instance(
        ...
        if max_distance_limit is not None:
            assert max_distance_limit > 0, "Distance limit must be greater than 0!"
        if speed is not None:
            assert max_time > 0, "Speed must be greater than 0!"
        if backhaul_class is not None:
            assert backhaul_class in (1, 2), "Backhaul class must be in [1, 2]!"

        assert (
            self.batch_size.numel() % n_augment == 0
        ), "Batch size must be divisible by n_augment!"
        ...
        for problem in VARIANT_PRESETS:
            for k in range(number_instances):
                ...
                if not os.path.exists(
                    ...
                    print(
                        f"Creating directory: data/generated/servs_{num_nodes - num_depots}_agents_{n_agent}/{problem}/validation"
                    )
                with open(
                    f"data/generated/servs_{num_nodes - num_depots}_agents_{n_agent}/{problem}/validation/"
                    + name
                    ...
                    print(
                        f"Dumped data into: data/generated/servs_{num_nodes - num_depots}_agents_{n_agent}/{problem}/validation/{name}.pkl"
                    )
                ...
                if not os.path.exists(
                    ...
                    print(
                        f"Creating directory: data/generated/servs_{num_nodes - num_depots}_agents_{n_agent}/{problem}/test"
                    )
                with open(
                    f"data/generated/servs_{num_nodes - num_depots}_agents_{n_agent}/{problem}/test/"
                    + name
                    ...
                    pickle.dump(instance, fp, protocol=pickle.HIGHEST_PROTOCOL)
                    print(
                        f"Dumped data into: data/generated/servs_{num_nodes - num_depots}_agents_{n_agent}/{problem}/test/{name}.pkl"
                    )
