...
class Environment(AECEnv):
    ...
    def _update_curr_agent_feasibility(self):
        ...
        time2depot = self.td_state["nodes"][
            "time2depot"
        ].clone()  # Time from nodes to depot
        ...
        exceeds_cap_linehaul = (
            ...
            + self.td_state["cur_agent"]["used_capacity_linehaul"]
            > self.td_state["agents"]["capacity"]
        )
        exceeds_cap_backhaul = (
            self.td_state["backhaul_demands"]
            + self.td_state["cur_agent"]["used_capacity_backhaul"]
            > self.td_state["agents"]["capacity"]
        )
        ...
        ) | (~exceeds_cap_backhaul & (self.td_state["backhaul_demands"] > 0))
        ...
        if self.force_visit:
            can_visit = ~(
                (self.td_state["cur_agent"]["cur_node_idx"] == 0).squeeze(-1)
                & (_mask[:, 1:].sum(-1) > 0)
            )
    ...
    def _update_all_agents_feasibility(self):
        ...
        active_nodes = (
            ...
            .unsqueeze(1)
            .expand(*batch_size, self.num_agents, self.num_nodes)
        )
        ...
        cur_node_coords = self.td_state["coords"].gather(
            1, cur_node_idx[:, :, None].expand(-1, -1, 2)
        )
        ...
        cur_coords_expanded = cur_node_coords.unsqueeze(2).expand(
            -1, -1, self.num_nodes, -1
        )
        all_coords_expanded = (
            self.td_state["coords"]
            .unsqueeze(1)
            .expand(*batch_size, self.num_agents, -1, -1)
        )
        distance2j = torch.sqrt(
            ((cur_coords_expanded - all_coords_expanded) ** 2).sum(dim=-1)
        )
        ...
        c2 = (
            torch.max(arrival_time, tw_low) + service_time + time2depot
        ) * ~open_routes <= end_time

        # Constraint 3: Agent does not exceed distance limit [B, A, N]
        cur_route_length = self.td_state["agents"]["route_length"].unsqueeze(
            -1
        )  # [B, A, 1]
        distance_limits = self.td_state["distance_limits"].unsqueeze(1)  # [B, 1, 1]
        c3 = (
            cur_route_length + distance2j + (distance2depot * ~open_routes)
            <= distance_limits
        )
        ...
        used_cap_l = self.td_state["agents"]["used_capacity_linehaul"].unsqueeze(
            -1
        )  # [B, A, 1]
        used_cap_b = self.td_state["agents"]["used_capacity_backhaul"].unsqueeze(
            -1
        )  # [B, A, 1]
    ...
    def _post_process_mask(self, mask):
        ...
        depot_idx_exp = (
            ...
            .unsqueeze(1)
            .expand(*batch_size, self.num_agents, 1)
        )  # [B, A, 1]
        ...
        if done_rows.numel() > 0:
            depot_idx = (
                self.td_state["depot_idx"].squeeze(-1)
                if self.td_state["depot_idx"].dim() > 1
                else self.td_state["depot_idx"]
            )
        ...
        if self.force_visit:
            ...
            has_unvisited = self.td_state["nodes"]["active_nodes_mask"][:, 1:].any(
                dim=-1
            )  # [B]
            must_leave = at_depot & has_unvisited.unsqueeze(-1)  # [B, A]
            depot_idx_expanded = depot_idx.unsqueeze(1).expand(
                *batch_size, self.num_agents, 1
            )  # [B, A, 1]
            depot_open = mask.gather(2, depot_idx_expanded) & ~must_leave.unsqueeze(
                -1
            )  # [B, A, 1]
    ...
    def _update_done(self, action):
        ...
        self.td_state["nodes"]["active_nodes_mask"].scatter_(
            1, action, action.eq(self.td_state["depot_idx"])
        )
    ...
    def _update_state(self, action):
        ...
        used_capacity_linehaul = (
            self.td_state["cur_agent"]["used_capacity_linehaul"]
            + selected_demand_linehaul
        )
        used_capacity_backhaul = (
            self.td_state["cur_agent"]["used_capacity_backhaul"]
            + selected_demand_backhaul
        )
        ...
        self.td_state["agents"]["used_capacity_linehaul"].scatter_(
            1,
            self.td_state["cur_agent_idx"],
            self.td_state["cur_agent"]["used_capacity_linehaul"],
        )
        self.td_state["agents"]["used_capacity_backhaul"].scatter_(
            1,
            self.td_state["cur_agent_idx"],
            self.td_state["cur_agent"]["used_capacity_backhaul"],
        )
        ...
        self.td_state["cur_agent"] = TensorDict(
            {
                ...
                .gather(1, self.td_state["cur_agent_idx"])
                .clone(),
                "used_capacity_linehaul": self.td_state["agents"][
                    "used_capacity_linehaul"
                ]
                .gather(1, self.td_state["cur_agent_idx"])
                .clone(),
                "used_capacity_backhaul": self.td_state["agents"][
                    "used_capacity_backhaul"
                ]
                .gather(1, self.td_state["cur_agent_idx"])
                .clone(),
    ...
    def check_solution_validity(self):
        ...
        distance2depot = get_distance(
            self.td_state["coords"], self.td_state["coords"][..., 0:1, :]
        )
        time2depot = distance2depot / self.td_state["speed"]
        if self.n_digits is not None:
        ...
        for ii in range(sorted_data.size(1)):
            ...
            curr_length = curr_length + dist * ~(
                self.td_state["open_routes"].squeeze(-1) & (next_node == 0)
            )  # Update curr_length
            ...
            assert torch.all(
                curr_time
                <= gather_by_index(self.td_state["time_windows"], next_node)[..., 1]
            ), "Agent must perform service before node's time window closes."
